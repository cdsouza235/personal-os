...
| HI-06 | `TODOIST_API_TOKEN` present in env (name-only) | P-RAIL-TD-02 | env/keychain | **provided** (D-PO-015, 2026-07-14: `PERSONALOS_RAIL_TODOIST_TOKEN` confirmed set on the Mac Mini, Chris's own Todoist account) |
| HI-07 | Gmail app password + sender + controlled recipient (names only) | P-RAIL-GM-02 | env/keychain | **provided** (D-PO-015, 2026-07-14: sender = sandboxed `cdsouza.bot@gmail.com`, controlled recipient = Chris's real inbox, all three env vars confirmed set on the Mac Mini) |
