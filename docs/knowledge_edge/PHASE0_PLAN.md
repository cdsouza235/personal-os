...
| P-KE-0C | Packet 0C | Synthetic golden fixtures under `tests/fixtures/knowledge_edge/` (or equivalent); no production tables, no scheduler, no Obsidian. **Probe scope is EDGAR + YouTube only (D-PO-019 drops FMP entirely) — see `PHASE0_PROBE_PLAN.md`, executed post-merge under direct Conductor-session supervision, not by any packet builder** | `tests/**` (fixtures only), read-only credentialed probes | per-packet | — | **S1** (uses Session 1 credentials); G3-adjacent (first credentialed use) |
...
| P-KE-3A | Packet 3A | `src/personalos/rails/knowledge_edge/earnings_calendar.py` (**adapter target is the roster/EDGAR/IR path per D-PO-019/`PHASE0_ROSTER.md` — not an FMP client**), company registry seeded from `PHASE0_ROSTER.md`, Tier A/B IR-root + IR/webcast vendor-domain list | `src/personalos/knowledge_edge/**`, `src/personalos/rails/knowledge_edge/**`, `migrations/**`, `docs/knowledge_edge/**` (domain-list record) | per-packet | — | **G5** + high-stakes (migrations) + **named async approval gate: "Packet 3A vendor-domain-list approval"** (Conductor must approve the assembled concrete domain list before Packet 3B fetches any redirect target on it — see `PHASE0_PROVIDERS_AND_ACCESS.md` §6; Session 1 approved only the redirect mechanism/rules, not this list) |
| P-KE-3B | Packet 3B | `src/personalos/rails/knowledge_edge/sec_edgar.py`, IR/webcast resolution | `src/personalos/rails/knowledge_edge/**`, `tests/**` | per-packet | — | **G5** (blocked on P-KE-3A's vendor-domain-list approval landing first — no live redirect fetch before that gate clears) |
...
| 3 | Every Tier A company verified; T-1→T-0→replay lifecycle proven in shadow | P-KE-3A's assembled IR/webcast vendor-domain list has landed the named "Packet 3A vendor-domain-list approval" gate (`PHASE0_PROVIDERS_AND_ACCESS.md` §6) **before** P-KE-3B fetches any redirect target on it; P-KE-3C shadow-mode E2E test log |
...
| `tests/test_knowledge_edge_rails_earnings_calendar.py` | Fake-client contract tests for the roster/EDGAR+IR earnings-calendar adapter (D-PO-019; not FMP — see `PHASE0_ROSTER.md`) |
