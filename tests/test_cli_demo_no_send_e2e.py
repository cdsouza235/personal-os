...
class DemoNoSendE2ECliTest(unittest.TestCase):
    def test_demo_no_send_e2e_cli_succeeds_with_safe_temp_output_dir(self) -> None:
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            self.assertEqual(payload["safety_assertions"]["readiness.status"], "not_ready")
            self.assertFalse(payload["safety_assertions"]["credentials_loaded"])
            self.assertFalse(payload["safety_assertions"]["credentials_read"])
            self.assertFalse(payload["safety_assertions"]["external_services_contacted"])
            self.assertFalse(payload["safety_assertions"]["openclaw_called"])
            self.assertFalse(payload["safety_assertions"]["scheduler_activated"])
